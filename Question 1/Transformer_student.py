...
class TransformerDecoder(layers.Layer):
    ...
    def call(self, inputs, encoder_outputs, mask=None):
        if mask is not None:
            padding_mask = tf.cast(mask[:, None, :], dtype="int32")
            look_ahead_mask = tf.linalg.band_part(tf.ones_like(padding_mask), -1, 0)
            combined_mask = tf.minimum(padding_mask, look_ahead_mask)
        else:
            combined_mask = None
...
decoder = tf.keras.Model([decoder_inputs, encoder_outputs], decoder_outputs)
...
# Add final dense layer to match vocabulary size
final_output = layers.Dense(vocab_size, activation="softmax")(decoder_outputs)
transformer = tf.keras.Model(
    [encoder_inputs, decoder_inputs], final_output, name="transformer"
)
...
def decode_sequence(input_sentence):
    ...
    # Generate tokens until we get an end token or reach max length
